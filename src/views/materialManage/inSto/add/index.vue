...
const submit = async (status:number) => {
    ...
    try {
        verfyArr.forEach((item:any)=>{
            ...
            if(temp.length>1){
                ...
                if(num>temp[0].max)
                    // message.error('库位'+temp[0].locationName+'入库数量超过库存最大值')
                    bool = true
                    throw new Error('库位'+temp[0].locationName+'入库数量超过库存最大值')
            }
            console.log(item);
            ...
            if(!item.stockNum || !item.location || !item.storeId || !item.areaId || !item.materialId || !item.projectId || !item.supplierId) throw new Error('请将订单信息填写完整')
        })
    } catch (error:any) {
    ...
    const result = await addInSto(params)
    if(result.status == 1) {
...
const getOrderNo = async ():Promise<orderResult> => {
    const result = await genOrderNum('CGRK')
    if(result?.status == 1) {
        return {
