...
import { getAllInSto, delInStoByIds
        } from '@/api/materialManager/inSto'
...
const getTableData = async () => {
    ...
    const result = await getAllInSto(params)
    if(result.status == 1) {
...
const deleteBtn = async () => {
    ...
    if(checkedKeys.value.length != 0) {
        const result = await delInStoByIds({ids: checkedKeys.value})
        if(result.status == 1) {
...
//table 新增按钮
const addBtn = () => {
    router.push({path: '/material/inSto/add'})
}
